...
(function ($) {
   ...
   */
	 ...
	 * 获取treetable 列集合
	 */
	$.fn.getTreeTableCols = function () {
		var colsArr = new Array();
		var treetable_cols = $(this).next(".fsTreeTableCols");
		if (!$.isEmpty(treetable_cols)) {
			$.each(treetable_cols.children(), function (i, n) {
				var _this = $(this);
				var col = {};

				if (!$.isEmpty(_this.attr("name"))) {
					col["name"] = _this.attr("name");
				}
				if (!$.isEmpty(_this.attr("treeNodes"))) {
					col["treeNodes"] = _this.attr("treeNodes");
				}
				if (!$.isEmpty(_this.attr("style"))) {
					col["style"] = _this.attr("style");
				}
				if (!$.isEmpty(_this.attr("headerClass"))) {
					col["headerClass"] = _this.attr("headerClass");
				}
