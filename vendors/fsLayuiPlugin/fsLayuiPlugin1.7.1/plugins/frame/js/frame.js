 ...
 * @License：MIT
 */
layui.use(['fsForm', 'fsDatagrid', 'fsTree', 'fsTreeTable', 'fsCommon', 'element'], function () {
	var fsForm = layui.fsForm,
		fsDatagrid = layui.fsDatagrid,
		fsTree = layui.fsTree,
		fsTreeTable = layui.fsTreeTable,
	...
	if (treetableDoms.length > 0) {
		$(treetableDoms).each(function (index, domEle) {
			var treetableId = $(this).attr("id");
			treetable = fsTreeTable.render({ id: treetableId });
			if (treetableDoms.length == 1) {
				treetables[treetableId] = treetable;
			} else {
				treetables[treetableId] = $.extend(true, {}, treetable);
			}
		});

		fsCommon.buttonEvent("treetable", getTreeTable);
	}

	function getTreeTable(treetableId) {
		if ($.isEmpty(treetables)) {
			fsCommon.warnMsg("未配置treetable！");
			return;
		}
		if ($.isEmpty(treetableId)) {
			treetableId = "treeDemo";
		}
		return treetables[treetableId];
	}
	/********* treetable 处理   end *************/
});